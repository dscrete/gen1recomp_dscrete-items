...
-- encounter.fishing modifiers such as Species Whistle replace the already
-- selected native catch with probability `chance`. reserveSpecies implements
-- exactly that mixture: native weights keep (1-chance), target gets `chance`.
function PokedexChip.applyFishingReplacement(Weights,dist,species,chance)
  if not species then return dist end
  return Weights.reserveSpecies(dist,species,chance)
end
...
local function total(dist)
  local value=0
  for _,weight in pairs(dist or {}) do value=value+(tonumber(weight) or 0) end
  return value
end
...
function PokedexChip.install(mod,runtime,Weights,deps)
  ...
  function PokedexChip.scan(game,species)
    ...
    for _,mapId in ipairs(ids) do
      ...
      -- Old/Good Rod pools are global, but only list them on maps that have
      -- a known fishable encounter source (water table or Super Rod group).
      ...
      for _,rod in ipairs({"OLD_ROD","GOOD_ROD","SUPER_ROD"}) do
        ...
        if candidates and #candidates>0 then
          ...
          if MysteryLure and runtime:isActive(MysteryLure.EFFECT_ID) then
            local selected=MysteryLure.selectedFor and MysteryLure.selectedFor(mapId,"fishing")
            dist=PokedexChip.applyFishingReplacement(Weights,dist,selected,
              MysteryLure.resolveShare(mod.options:get(MysteryLure.SHARE_OPTION)))
          elseif SpeciesWhistle and runtime:isActive(SpeciesWhistle.EFFECT_ID) then
            local target=SpeciesWhistle.target and SpeciesWhistle.target()
            if target then
              if dist[target] then
                local rate=math.min(0.80,0.15*SpeciesWhistle.resolveStrength(
                  mod.options:get(SpeciesWhistle.STRENGTH_OPTION)))
                dist=PokedexChip.applyFishingReplacement(Weights,dist,target,rate)
              elseif SpeciesWhistle.nonlocalEnabled and SpeciesWhistle.nonlocalEnabled(mod) then
                dist=PokedexChip.applyFishingReplacement(Weights,dist,target,
                  SpeciesWhistle.resolveNonlocalRate(mod.options:get(SpeciesWhistle.NONLOCAL_RATE_OPTION)))
              end
            end
          end
          if dist[species] then
            ...
            for _,row in ipairs(candidates) do
              local level=tonumber(row.level)
              if level and row.species==species then
                lo=lo and math.min(lo,level) or level
                hi=hi and math.max(hi,level) or level
              end
            end
  ...
  function Screen:draw()
    ...
    for i=0,2 do
      ...
      if row then
        local y=38+i*30
        Font.draw(row.mapName:sub(1,18),12,y)
        if first+i==self.cursor then Font.drawCode(Theme.cursor,4,y) end
