...
local function newTrackerScreen(game,rows,emptyText)
  ...
  function Screen:draw()
    ...
    if #self.rows==0 then
      ...
      if last<#self.rows then Font.drawCode(Theme.moreArrow,144,120) end
    end

    Font.draw("CLOSE",TRACKER_NAME_X,TRACKER_CLOSE_Y)
    if self.closeSelected then Font.drawCode(Theme.cursor,TRACKER_CURSOR_X,TRACKER_CLOSE_Y) end
