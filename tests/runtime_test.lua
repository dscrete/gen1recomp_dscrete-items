...
test("Safari Kit session decision survives runtime recreation", function()
  local s = store()
  local first = Runtime.new(s)
  first:setReusableState("safari_kit", "bait")
  local restored = Runtime.new(s)
  eq(restored:getReusableState("safari_kit", nil), "bait")
  eq(s.values.reusable_safari_kit, "bait")
end)

test("Prototype Resonator active state survives runtime recreation", function()
  local s = store()
  local first = Runtime.new(s)
  first:activateFieldEffect("prototype_resonator", 250)
  first:onEligibleStep()
  local restored = Runtime.new(s)
  check(restored:isActive("prototype_resonator"))
  eq(restored.remainingSteps, 249)
end)
...
test("persistent reset clears Safari Kit session lock", function()
  local s = store()
  local runtime = Runtime.new(s)
  runtime:setReusableState("safari_kit", "pass")
  runtime:resetPersistent({ "safari_kit" })
  eq(runtime:getReusableState("safari_kit", nil), nil)
  eq(s.values.reusable_safari_kit, nil)
end)
